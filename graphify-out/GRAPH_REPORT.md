# Graph Report - faceguard  (2026-09-04)

## Corpus Check
- 151 files · ~230,299 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1126 nodes · 1509 edges · 132 communities (80 shown, 52 thin omitted)
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 238 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f5dc91b5`
...
1. `FakeSession` - 29 edges
2. `FakeAttendanceClient` - 22 edges
3. `FakeExecuteResult` - 22 edges
4. `MediaPipeActiveLivenessChecker` - 22 edges
5. `RecordingIndex` - 20 edges
6. `frontend/AGENTS.md — Public Site` - 18 edges
7. `ActiveLivenessChallenge` - 17 edges
8. `AGENTS.md — Root Instructions for Claude Code` - 15 edges
9. `BackendClient` - 14 edges
10. `crawler/AGENTS.md — Web Crawler` - 14 edges
...
- `test_frame_and_observation_are_typed_transient_values()` --calls--> `BoundingBox`  [INFERRED]
  tests/unit/test_headless_contracts.py → src/faceattend/vision/types.py
- `test_frame_and_observation_are_typed_transient_values()` --calls--> `FaceQuality`  [INFERRED]
  tests/unit/test_headless_contracts.py → src/faceattend/vision/types.py
- `test_current_pipeline_rejects_no_face()` --calls--> `get_single_face()`  [INFERRED]
  tests/characterization/test_current_face_selection.py → ml/pipeline/face.py
...
Cohesion: 0.05
Nodes (68): Characterize face-count behavior that the desktop pipeline must preserve., test_current_pipeline_accepts_exactly_one_face(), test_current_pipeline_rejects_multiple_faces(), test_current_pipeline_rejects_no_face(), ActiveLivenessChecker, Active liveness challenge/response checks., LivenessResult, ActiveLivenessChallenge (+60 more)

### Community 1 - "Fake Session"
Cohesion: 0.09
Nodes (36): LivenessChecker, LivenessChecker interface., Per-platform takedown integration interface., Submit the notice; return the platform's reference id + status., TakedownPlatform, Protocol, StrEnum, test_frame_and_observation_are_typed_transient_values() (+28 more)
...
Cohesion: 0.08
Nodes (31): Append-Only Biometric Audit Log, ArcFace R100 Embedding, Current Attendance API Scope, Biometric Data Minimization, Current Enrollment and Search Pipeline, Deepfake Liveness Check, Versioned 512-Dimensional Face Embedding, Embedding Model Version Display (+23 more)

### Community 10 - "Fast Api Backend Api"
Cohesion: 0.07
Nodes (30): API Key Authentication, Backend Services, Legacy Crawl Override Log, Crawler Image Download, ImageItem, Crawler Inference Queue, Crawler S3 or MinIO Storage, Crawler Perceptual-Hash Deduplication (+22 more)
...
Cohesion: 0.1
Nodes (21): _evenly_sample(), Helpers for sampling frames from short webcam videos., Raised when a check-in video cannot be sampled., sample_video_frames_as_jpegs(), VideoFrameDecodeError, AttendanceState, new_attendance_state_machine(), new_registration_state_machine() (+13 more)
...
Cohesion: 0.11
Nodes (17): code:text (DATE — Phase N — STATUS), Current decision record, FaceAttend Local-First Migration Plan, Latest execution record, Phase 0 — Protect and characterize the existing project, Phase 1 — Define the headless package boundary, Phase 2 — Extract and validate the CV core, Phase 3 — Add local SQLite persistence (+9 more)
...
Cohesion: 0.29
Nodes (5): embed(), liveness_active(), _raise_for_active_liveness_service_error(), Thin HTTP wrapper for the non-Triton serving path (port 8003).  Selected by INFE, Map invalid requests and service problems to HTTP errors.
...
Cohesion: 0.53
Nodes (5): main(), _print_embedding(), _print_liveness(), Smoke test the local face pipeline without running the ML HTTP service.  Usage:, _read_image()
...
## Knowledge Gaps
- **344 isolated node(s):** `Public site (port 8000) — end-user-facing FastAPI + Jinja2 + HTMX app.`, `Public site auth routes — /login, /register, /verify.`, `/billing — Stripe-backed plan management.`, `/attendance — attendance result dashboard + detail.`, `/face-registration — register a consenting face for attendance.` (+339 more)
...
- **Why does `VectorStore` connect `Faissstore` to `Fake Session`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `Match` connect `Threshold Calibration` to `Base Model`, `Faissstore`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `VideoFrameDecodeError` connect `Attendance.Py` to `Threshold Calibration`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Are the 8 inferred relationships involving `FakeSession` (e.g. with `EmbeddingResult` and `LivenessCheck`) actually correct?**
  _`FakeSession` has 8 INFERRED edges - model-reasoned connections that need verification._
- **Are the 8 inferred relationships involving `FakeExecuteResult` (e.g. with `EmbeddingResult` and `LivenessCheck`) actually correct?**
  _`FakeExecuteResult` has 8 INFERRED edges - model-reasoned connections that need verification._
- **Are the 12 inferred relationships involving `MediaPipeActiveLivenessChecker` (e.g. with `LivenessResult` and `ActiveLivenessChallenge`) actually correct?**
  _`MediaPipeActiveLivenessChecker` has 12 INFERRED edges - model-reasoned connections that need verification._
- **Are the 8 inferred relationships involving `RecordingIndex` (e.g. with `EmbeddingResult` and `LivenessCheck`) actually correct?**
  _`RecordingIndex` has 8 INFERRED edges - model-reasoned connections that need verification._